...
export const StreamDetailPage = () => {
  ...
  const handleWithdraw = async () => {
    ...
    const success = await withdrawFromStream(
      id!,
      streamDetails.withdrawableAmount,
    );
    if (success) {
      fetchStreamDetails();
    }
      ...
      <Layout>
        <div className="flex items-center justify-center h-full">
          <div className="text-center py-24 px-8 bw-card max-w-2xl bw-shadow">
            ...
            <p className="text-muted-foreground mb-8">
              The stream with ID {id} could not be found.
            </p>
            <button
      ...
      <div className="max-w-4xl mx-auto space-y-6 p-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-foreground">
                Stream #{streamDetails.id}
              </h1>
              <p className="text-muted-foreground">
                Stream details and management
              </p>
            </div>
          </div>
          <div className="status-active px-3 py-1 rounded text-xs font-medium">
            Active
          </div>
        </div>
        ...
        <div className="bw-card p-6 space-y-6 bw-shadow">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InfoItem
              label="Sender"
              value={streamDetails.sender}
              isAddress={true}
            />
            <InfoItem
              label="Recipient"
              value={streamDetails.recipient}
              isAddress={true}
            />
            <InfoItem
              label="Total Amount"
              value={formatAmount(
                streamDetails.totalAmount,
                streamDetails.tokenDecimals,
            ...
            <InfoItem
              label="Remaining"
              value={formatAmount(
                streamDetails.remainingBalance,
                streamDetails.tokenDecimals,
                streamDetails.tokenSymbol,
              )}
            />
            <InfoItem
              label="Start Time"
              value={formatDate(streamDetails.startTime)}
            />
            <InfoItem
              label="Stop Time"
              value={formatDate(streamDetails.stopTime)}
            />
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm font-mono bw-text-accent">
                {progress.toFixed(2)}%
              </span>
            </div>
          ...
          <div className="space-y-3">
            {address?.toLowerCase() ===
              streamDetails.recipient.toLowerCase() && (
              <button
              ...
              <>
                <div className="flex gap-4 w-full">
                  <Input
                    ...
                    className="flex-1 bw-input w-full"
                  />

                  <button
                    onClick={handleTopUp}
                    disabled={loading || !topUpAmount}
                    className="w-[20%] h-10 text-sm flex justify-center items-center bw-button-primary disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Top Up
                  </button>
                </div>
